...
生成内容池前，必须先对原文做统一取舍。这个取舍不分 PPT、信息图、H5，三个子产物都继承同一套筛选结果。

### 直接剔除

以下内容不进入正文级内容池，只在极端复盘场景下才可能作为背景：

- 对话拉扯、多方辩论、反复试探的中间过程。
- 临时发散、无落地价值的闲聊、口头吐槽、无关题外话。
- 内部失败试错、踩坑过程、无正向结论的无效尝试。
- 多层推导、长篇论证过程；通常只保留最终推导结论。

### 压缩简化

以下内容只保留最终结果或影响结论的必要中间信息：

- 调研、访谈、实验的完整执行流程，压缩为数据、发现和结论。
- 多方协商、多轮修改的完整过程，压缩为当前定稿版本和关键分歧。
- 复杂背景铺垫、冗长举例，通常用 1 句话概括。
- 如果推导、调研或协商过程中的中间信息会影响最终判断、风险或行动，必须保留该中间信息。

### 完整保留

以下内容是内容池核心素材，不能因“凝练”而删掉：

- 顶层核心观点、总定位、核心目标、底层原则。
- 定型方案、标准化规则、完整落地或执行路径。
- 对结论、行动、边界有影响的量化信息：成本、时效、规模、增长率、样本量、指标阈值等。弱相关数字不要喧宾夺主。
- 分层分类体系、不同维度的对比标准。
- 优势、壁垒、风险、约束条件、应对策略。
- 分阶段计划、权责分工、交付节点。
- 核心证据、关键结论、可复用方法论。
...
1. 完整阅读原文，按“直接剔除 / 压缩简化 / 完整保留”三层规则完成前置筛选。
